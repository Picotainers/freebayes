...
# Compatibility-first template for freebayes.
...
RUN set -eux; \
    BIN=""; \
    if [ -x "/opt/conda/bin/freebayes" ]; then BIN="/opt/conda/bin/freebayes"; fi; \
    if [ -z "$BIN" ]; then CAND="/opt/conda/bin/$(echo freebayes | tr '-' '_')"; [ -x "$CAND" ] && BIN="$CAND" || true; fi; \
    if [ -z "$BIN" ]; then BIN="$(find /opt/conda/bin -maxdepth 1 -type f -perm -111 -name 'freebayes*' | head -n1 || true)"; fi; \
    test -n "$BIN"; \
...
RUN set -eux; \
    ...
    printf '#!/usr/bin/env bash\nexec "%s" "$@"\n' "$BIN" > /usr/local/bin/freebayes
RUN chmod +x /usr/local/bin/freebayes && rm -f /tmp/tool-entry-path
WORKDIR /data
